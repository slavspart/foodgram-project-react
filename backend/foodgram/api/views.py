...
from .serializers import FavoriteSerializer, IngredientSerializer, RecipeCreateSerializer, RecipeSerializer, ShoppingCartSerializer, TagSerializer
...
class RecipeViewSet(
    ...
    def get_queryset(self):
        ...
        if tags != []:
            queryset = queryset.filter(tags__slug__in=tags).distinct().order_by('-id')
    ...
    # перебираем объекты списка покупок у данного юзера
        ...
        # перебираем ингредиенты из объектов shopping_cart
            if (f'{ingredient.ingredient.name}'
                f'({ingredient.ingredient.measurement_unit}) -') not in dict:
